...
Wenn wir uns einen Computer als Gesprächspartner vorstellen und wollten mit diesem sprechen, so müssten wir zuerst einmal eine Programmiersprache lernen. Genauso wie man wenn mit Annika\footnote{Person erfunden}, welche Italienisch, Deutsch und Englisch verstehen kann, sprechen will, einer ihrer Sprachen lernen muss. Also lerne ich zum Beispiel Python und teile dem Computer die Botschaft $print(2 + 5)$ mit. Ich habe nun, nach Wittgenstein die Bedeutung dieses Ausdrucks durch Beispiele im Gebrauch gelernt.  \\
...
\subsection{Das Käfer-Gleichnis}
\begin{displayquote}
\enquote{
...
\end{displayquote}
Wenn wir nun dieses Zitat Wittgensteins auf das Wort \enquote{Sprachverständnis} beziehen, so sehen wir ein Problem mit dem Appell auf eine objektive Definition für die inneren Zustände, wenn man eine Sprache versteht. Wie kann Person A wissen, ob in ihr die gleichen Vorgänge vorgehen wie in Person B, wenn sie beide eine Frage beantworten? \\
...
\\
Man kann dieses Kapitel als eine Leiter Wittgensteins sehen, die man wegstossen muss, sobald man hinaufgeklettert ist.\footnote{Wittgenstein 1921, §6.54} So bringt uns das Käfer-Gleichnis nämlich wieder zurück zur Leitfrage. \\
...
Warum nennt man Programmiersprachen also Sprachen? Wenn man die Suche nach einem absoluten Objekt aufgibt, bleibt noch eine andere Möglichkeit als aufzugeben? Kann nicht jedes Wort für jedes Ding angewendet werden? Bedenken wir ein Beispiel: \\
\\
...
Die Übereinstimmung ist nicht sofort ersichtlich: Denn ist für Wittgenstein ein Sprachspiel durch den Gebrauch definiert, während die Libraries ihre Funktionen unabhängig von deren Gebrauch besitzen. Oder zumindest scheinen sie unabhängig vom Gebrauch, bis man bedenkt, dass eine Funktion noch immer von einer Person geschrieben werden muss. Diese Person will die Funktion für eine Kurzfassung verschiedener Rechnungschritte \textbf{gebrauch}en. Das bedeutet eine Existenz einer Funktion bedingt immer einen beabsichtigten Gebrauch.\footnote{Diese Aussage wirft neue Fragen über Funktionen und deren Abhängigkeit von dessen Autors Absicht auf. Es eröffnen sich nicht nur neue Fragen sondern ein gesamter Abschnitt der Auteur-Theorie auf, welche man auch auf geschriebenen Code beziehen kann. Es existiert jedoch kaum Literatur zu diesen Thema und würde es den Rahmen dieser Facharbeit sprengen.} \\
